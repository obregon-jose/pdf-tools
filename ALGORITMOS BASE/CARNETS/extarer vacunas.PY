...
Se agregó también la opción para procesar recursivamente una carpeta con archivos .pdf
(Procesar carpeta (PDFs recursiva)).
"""
import re
import threading
import json
...
# ---------------- Nueva función: procesar carpeta de PDFs recursiva ----------------
def process_pdf_folder_and_append(pdf_folder: Path, output_xlsx: Path, ocr_fallback=True, ocr_force=False, ui_callback=None):
    """
    Recorre recursivamente pdf_folder buscando archivos .pdf, procesa cada PDF
    y añade la fila correspondiente al output_xlsx. ui_callback (si se pasa) se
    llama con mensajes para la GUI.
    """
    pdf_folder = Path(pdf_folder)
    output_xlsx = Path(output_xlsx)
    if not pdf_folder.exists():
        raise FileNotFoundError(pdf_folder)
    output_xlsx.parent.mkdir(parents=True, exist_ok=True)

    processed = 0
    # Buscar todos los PDFs recursivamente
    pdfs = sorted([p for p in pdf_folder.rglob("*.pdf") if p.is_file()])
    if ui_callback:
        ui_callback(f"Encontrados {len(pdfs)} PDFs en {pdf_folder}")
    for pdf in pdfs:
        try:
            if ui_callback:
                ui_callback(f"Analizando PDF: {pdf.relative_to(pdf_folder)}")
            res = analyze_pdf(str(pdf), ocr_fallback=ocr_fallback, ocr_force=ocr_force)
            row = make_row_from_result(res)
            append_row_to_excel(str(output_xlsx), row)
            processed += 1
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("Extractor -> Excel columnas separadas (PDF individuel / ZIP folder / PDF folder)")
        ...
        # Nuevo botón para procesar carpeta de PDFs recursiva
        self.select_pdf_folder_btn = tk.Button(frm, text="Procesar carpeta (PDFs recursiva)", command=self.select_pdf_folder)
        self.select_pdf_folder_btn.pack(side=tk.LEFT, padx=6)
        ...
        self.status_label = tk.Label(root, text="Seleccione un PDF o una carpeta ZIP/PDF para empezar.")
    ...
    def select_pdf_folder(self):
        folder = filedialog.askdirectory(title="Seleccionar carpeta con PDFs (recursiva)")
        if not folder:
            return
        out_fn = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files","*.xlsx")], title="Guardar archivo Excel de salida")
        if not out_fn:
            return
        self.status_label.config(text=f"Procesando carpeta de PDFs: {folder} ...")
        self.output.delete("1.0", tk.END)
        self.export_btn.config(state=tk.DISABLED)
        self.save_btn.config(state=tk.DISABLED)
        th = threading.Thread(target=self._process_pdf_folder_thread, args=(folder, out_fn, self.ocr_var.get()), daemon=True)
        th.start()
    ...
    def _process_pdf_folder_thread(self, folder, out_fn, ocr_force):
        try:
            def ui_cb(msg):
                self.root.after(0, lambda: self.output.insert(tk.END, msg + "\n"))
            processed = process_pdf_folder_and_append(Path(folder), Path(out_fn), ocr_fallback=True, ocr_force=ocr_force, ui_callback=ui_cb)
            self.root.after(0, lambda: self.status_label.config(text=f"Procesado carpeta PDFs: {folder} -> {processed} PDFs añadidos a {out_fn}"))
            self.root.after(0, lambda: messagebox.showinfo("Procesado", f"Procesados: {processed} PDFs\nSalida: {out_fn}"))
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Error", f"Ocurrió un error: {e}"))
            traceback.print_exc(file=sys.stderr)
